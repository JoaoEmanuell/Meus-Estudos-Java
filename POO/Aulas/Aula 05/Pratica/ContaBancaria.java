public class ContaBancaria {
    ...
    public ContaBancaria(Integer numbercont ,String tip, String owner, Double balance){
        ...
        setBalance(balance);
        setOwner(owner);
        if (openAccount(tip).equals("FALSE")){
            setTip("null");
        }
    }
    ...
    public String openAccount(String t){
        t = t.toUpperCase();
        this.setTip(t.toUpperCase());
        setIsOpen(true);
        if (t.equals("CC")){
            return getTip() + " " + this.isOpen;
        } else if (t.equals("CP")){
            return getTip() + " " + this.isOpen;
        } else{
            return "FALSE";
        ...
        if (!isOpen){
            ...
            if(getBalance() < 12.0) {
                return "Valor da conta insuficiente para pagar a mensalidade, por favor realize um deposito";
                    ...
                    acutalbalance -= 12.00;
                    setBalance(acutalbalance);
                    return "R$ 12.00 foram descontados da sua conta";
                }
        } else if(getTip().equals("CP")){
            if(getBalance() < 20.00) {
                return "Valor da conta insuficiente para pagar a mensalidade, por favor realize um deposito";
            } else {
                Double acutalbalance = getBalance();
                acutalbalance -= 20.00;
                setBalance(acutalbalance);
                return "R$ 20.00 foram descontados da sua conta";
            }
        } else {
            return "FALSE";
        }
